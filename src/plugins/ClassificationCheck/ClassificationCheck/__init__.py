...
The ClassificationCheck-class is imported from both run_plugin.py and run_debug.py
"""
import sys
import logging
...
# Setup a logger
logger = logging.getLogger('ClassificationCheck')
...
# class ClassificationCheck(PluginBase):
#     def main(self):
...
#     isWorkflowNet()
   ...
   # logger.info(json.dumps(places,indent=4))
    ...
    # self.send_notification('ends {0}/nfreeChoice {1}/nstateMachine {2}/nmarkedGraph {3}n/workflowNet {4}'.format(ends, freeChoice, stateMachine, markedGraph, workflowNet ) 
    self.send_notification('everything is OK')
